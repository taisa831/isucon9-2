...
.PHONY: log
log:
	sudo journalctl -u isucari.golang -n100 -f

.PHONY: maji
maji: commit before build restart

# MySQLログとNginxログをSlackに投げる
.PHONY: anal
anal: slow kataru
...
commit:
	...
	cd $(BUILD_DIR); \
	git add .; \
	git commit --allow-empty -m "tuning"
...
.PHONY: setup
setup:
	sudo apt install -y percona-toolkit dstat git zip unzip snapd
	# git setup
